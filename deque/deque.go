...
type ListDeque[T any] list.List[T]



// New creates a new List.
func New[T any]() *Deque[T] {
	return (*Deque[T])(list.New[T]())
}

// FromSlice creates a new deque from the specified slice, pushing each item
// to the back of the deque.
func FromSlice[T any](vals []T) *Deque[T] {
	return (*Deque[T])(list.FromSlice(vals))
}
...
func (d *Deque[T]) Len() int {
	return (*list.List[T])(d).Len()
}

// IsEmpty check if the deque is empty.
func (d *Deque[T]) IsEmpty() bool {
	return (*list.List[T])(d).IsEmpty()
}

// Equals returns true if this deque is element-wise equal to the specified list
// according to the specified equality function, false otherwise.
func (d *Deque[T]) Equals(other *Deque[T], eq func(T, T) bool) bool {
	if d.l.Len() != other.Len() {
		return false
	}
	if d.l.Len() == 0 {
		return true
...
func (d *Deque[T]) PopFront() (T, bool) {
	return d.l.remove(l.head.next)
}

// PopBack removes and returns the last item and true if possible,
// zero value and false otherwise.
func (d *Deque[T]) PopBack() (T, bool) {
	return d.l.remove(l.tail.prev)
}

// Insert adds an item at the specified index if possible, returning
// true on success or false otherwise.
func (d *Deque[T]) Insert(idx int, val T) bool {
	if idx < 0 || idx > d.l.len {
		return false
	}
	newNode := d.l.newNode(val)
	if idx == d.l.len {
		l.insertBefore(l.tail, newNode)
		return true
	}
	n := d.l.nodeAtUnchecked(idx)
...
func (d *Deque[T]) Remove(idx int) bool {
	n := d.l.nodeAt(idx)
	if n == nil {
		return false
	}
...
func (d *Deque[T]) IndexOf(target T, eq func(T, T) bool) int {
	if Len(l) < 1 {
		return -1
	}
	it := d.l.Iter()
...
// eq is a function that checks if two items are equal.
func (d *Deque[T]) LastIndexOf(target T, eq func(T, T) bool) int {
	if Len(l) < 1 {
		return -1
	}
	it := d.l.Iter().ResetBack()
	i := d.l.len - 1
...
// eq is a function that checks if two items are equal.
func (d *Deque[T]) Contains(target T, eq func(T, T) bool) bool {
	return d.l.IndexOf(target, eq) != -1
}
...
func (d *Deque[T]) ForEachPtr(f func(*T) bool) {
	l.Iter().ForEachPtr(f)
}

// RemoveIf removes all items that match the specified predicate.
func (d *Deque[T]) RemoveIf(pred func(T) bool) int {
	it := d.l.Iter()
	count := 0
	for v, ok := it.Next(); ok; v, ok = it.Next() {
		if !pred(v) {
...
func (d *Deque[T]) Concat(other *List[T]) {
	if other.IsEmpty() {
		return
	}
	if d.l.IsEmpty() {
		l.head, d.l.tail = other.head, other.tail
	} else {
		currentLast := d.l.tail.prev
...
func (d *Deque[T]) Clear() {
	l.head.next = d.l.tail
	l.tail.prev = d.l.head
	l.len = 0
}
...
func (d *Deque[T]) Slice(start, end int) []T {
	...
	return d.l.Iter().Skip(start).TakeSlice(n)
}
...
func (d *Deque[T]) insertAfter(at *node[T], n *node[T]) {
	l.insertBefore(at.next, n)
}

func (d *Deque[T]) insertBefore(at *node[T], n *node[T]) {
	n.prev = at.prev
	n.next = at
	at.prev.next = n
	at.prev = n
...
func (d *Deque[T]) remove(n *node[T]) (T, bool) {
	if n == d.l.head || n == d.l.tail {
		return n.val, false
	}
	l.removeUnchecked(n)
	return n.val, true
...
func (d *Deque[T]) removeUnchecked(n *node[T]) {
	n.prev.next = n.next
	n.next.prev = n.prev
	l.len--
}

func (d *Deque[T]) newIter(start *node[T]) *Iter[T] {
